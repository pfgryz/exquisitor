...
fn calculate_search_quality(reference: Vec<OrganismFound>, probe: Vec<OrganismFound>) -> (f64, f64) {
    let reference_map: HashMap<String, f64> = reference.into_iter()
        .map(|s| s.to_tuple())
        .collect();

    let probe_map: HashMap<String, f64> = probe.into_iter()
        .map(|s| s.to_tuple())
        .collect();
    ...
    // Negative
    let unique: HashSet<String> = reference_map.keys()
